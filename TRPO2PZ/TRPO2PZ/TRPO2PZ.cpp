...
void byRandom(int numbersLine, int numbersColumn, int** matrix)
{
	...
	for (int counterLine = 0; counterLine < numbersLine; counterLine++)
	{
		for (int counterColumn = 0; counterColumn < numbersColumn; counterColumn++)
		{
			int znak = rand() % 3;//специально 3, чтобы отрицательных чисел было меньше. Мне так больше нравится
			int sign;
			if (znak == 1)
				sign = -1;
			else
...
void medianMatrixAndMinMax()
{
	...
	/*!!!!!!!!!!!Это я оставил на случай если минимальный и максимальный элемент вы хотели чтобы я нашел так!!!!!!!!!!!!!!!!
	
	
	int maxNumberOfArray = matrix[0][0];
	for (int i = 0; i < numberLine; ++i)
	{
		for (int j = 0; j < numberLine; ++j)
		{
			if (matrix[i][j] > maxNumberOfArray)
			{
				maxNumberOfArray = matrix[i][j];
			}
		}

	}
	int minNumberOfArray = matrix[0][0];
	for (int i = 0; i < numberLine; ++i)
	{
		for (int j = 0; j < numberLine; ++j)
		{
			if (matrix[i][j] < minNumberOfArray)
			{
				minNumberOfArray = matrix[i][j];
			}	
		}
...
int main()
{
	...
	cout << "Важная информация!!!!!! я k вам подходил и спросил: Можно ли объединять задания?\nВы сказали: Только так, чтобы я об этом знал. Так вот, я напоминаю\n";
	int toggle;
	while (true) {
		do {
